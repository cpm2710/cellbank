...
static int spawn_writers(void)
{
    unsigned int t = nb_writers;
    
    do {
        t--;
	ifp ff=&writerArray[t];
	ff->index=t;
	
	printf("ff->flag==%u\n",ff->flag);
	printf("ff->index==%u\n",ff->index);
        if (pthread_create(&writers_threads[t], NULL, writer, ff) != 0) {
            return -1;
        }
    } while (t != 0U);
...
static int spawn_rewriters(void)
{
...
static int wait_rewriters(void)
{
    unsigned int t = nb_rewriters;
...
static int spawn_commenters(void)
{
    unsigned int t = nb_commenters;

    do {
        t--;
        if (pthread_create(&commenters_threads[t], NULL,
                           commenter, NULL) != 0) {
            return -1;
        }
    } while (t != 0U);
...
static int wait_commenters(void)
{
    unsigned int t = nb_commenters;
...
static void show_grand_total(void)
{
    reentrant_printf("\n"
...
static int init_threads_area(void)
{
    if ((writers_threads =
         malloc(nb_writers * sizeof writers_threads[0])) == NULL ||
        (rewriters_threads =
         malloc(nb_rewriters * sizeof rewriters_threads[0])) == NULL ||
        (readers_threads =
         malloc(nb_readers * sizeof readers_threads[0])) == NULL ||
        (commenters_threads =
         malloc(nb_commenters * sizeof commenters_threads[0])) == NULL) {
...
int process(void)
{
    ...
    reentrant_printf("Benchmarking for %u iterations.\n"
    ...
    do {
        ...
        reentrant_printf
            ("%10llu %12llu %13llu %13llu %13llu %13llu %13llu\n",
             stats_total_blogs_write,
             stats_articles_read, stats_articles_write,
             stats_pictures_read, stats_pictures_write,
             stats_comments_read, stats_comments_write);
