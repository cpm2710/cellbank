...
int main(int argc, char *argv[])
{
	...
	readerArray=(ifp)calloc(nb_readers, sizeof(indexFl));
	int index=0;
	for(;index<nb_readers;index++){
		readerArray[index].index=0;
		readerArray[index].flag=1;
	}
	writerArray=(ifp)calloc(nb_writers,sizeof(indexFl));
	index=0;
	for(;index<nb_writers;index++){
		writerArray[index].index=0;
		writerArray[index].flag=2;
	}
	rewriterArray=(ifp)calloc(nb_rewriters,sizeof(indexFl));

	index=0;
	for(;index<nb_rewriters;index++){
		rewriterArray[index].index=0;
		rewriterArray[index].flag=3;
	}
	commenterArray=(ifp)calloc(nb_commenters,sizeof(indexFl));

	index=0;
	for(;index<nb_commenters;index++){
		commenterArray[index].index=0;
		commenterArray[index].flag=4;
	}
    if (process() != 0) {
        return 1;
    }    
