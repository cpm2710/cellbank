...
static int create_blog(ifp p)
{
    ...
    if (create_articles(file_name, sizeof file_name, blog_id,p) != 0 ||
        create_pictures(file_name, sizeof file_name, blog_id,p) != 0) {
        return -1;
    }
    stats_blogs_write++;
...
void *writer(void * const fodder,ifp p)
{    
    (void) fodder;
    do {
        if (create_blog(p) != 0) {
